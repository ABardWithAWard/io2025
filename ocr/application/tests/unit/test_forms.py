...
class UploadFileFormTests(TestCase):
    ...
    def test_valid_jpg_file(self):
        file = SimpleUploadedFile('test.jpg', b'\xff\xd8\xff\xe0', content_type='image/jpeg')
        form = UploadFileForm(files={'file': file})
        self.assertTrue(form.is_valid())

    def test_valid_jpeg_file(self):
        file = SimpleUploadedFile('test.jpeg', b'\xff\xd8\xff\xe1', content_type='image/jpeg')
        form = UploadFileForm(files={'file': file})
        self.assertTrue(form.is_valid())
    ...
    def test_valid_webp_file(self):
        file = SimpleUploadedFile('test.webp', b'RIFF....WEBP', content_type='image/webp')
        form = UploadFileForm(files={'file': file})
        self.assertTrue(form.is_valid())
